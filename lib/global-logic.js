...
function terminated() {
    ...
    // Start from the bottom and loop through every row upwards
    for (let y = appHeight; y > 0; y -= unit) {
        ...
        // If all cells are filled- then remove the row
        if(matchingIndexes == maxIndexes) {
            // Find all blocks on the current line
            var filtered = Game.inactiveTetroninos.filter(function(value, index, arr){ 
                return value.y == y;
            });
            // Find all block 'not' on the current line
            var remaining = Game.inactiveTetroninos.filter(function(value, index, arr){ 
                return value.y != y;
            });

            // De-render the sprites for all the affected blocks
            filtered.forEach(sprite => {
                app.stage.removeChild(sprite);
            });

            // Reassign all the blocks that are not affected by this 
            Game.inactiveTetroninos = remaining;

            // Move every line above the current line down by one 'y' position
            Game.inactiveTetroninos.filter(f => f.y < y).forEach(remainingBlock => {
                remainingBlock.y += unit;
            });
        }
    }
